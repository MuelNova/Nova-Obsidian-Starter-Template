...
const score = count > 5 ? 1 : count * 0.2
...
let score = count >= 5 ? 0.8 : (count - 1) * 0.2
if (score < 0) {
	score = 0
}
score += riceCount >= 20 ? 0.2 : riceCount * 0.01
const res = "💯得分：" + score.toFixed(2) + " / 1.00\n👏记录次数：" + count + "\n🍚额外米饭：" + riceCount;
dv.el('div', res, {cls: score >= 0.8 ? 'score-tier1' : score >= 0.5 ? 'score-tier2' : 'score-tier3'})
```
...
const score = count >= 14 ? 1.00 : count / 14
const res = "💯得分：" + score.toFixed(2) + " / 1.00\n⏱️总时长：" + count + "h";
dv.el('div', res, {cls: score >= 0.8 ? 'score-tier1' : score >= 0.5 ? 'score-tier2' : 'score-tier3'})
...
const score = count >= 7 ? 1.00 : count / 7
const res = "💯得分：" + score.toFixed(2) + " / 1.00\n⏱️总时长：" + count + "h";
dv.el('div', res, {cls: score >= 0.8 ? 'score-tier1' : score >= 0.5 ? 'score-tier2' : 'score-tier3'})
...
```toggl
LIST
FROM <% moment(tp.file.title, 'gggg-[W]ww').weekday(0).format('YYYY-MM-DD') %> TO <% moment(tp.file.title, 'gggg-[W]ww').weekday(6).format('YYYY-MM-DD') %>
GROUP BY PROJECT
SORT DESC
```
```toggl
